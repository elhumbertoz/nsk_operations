...
class MergePurchaseAutomatic(models.TransientModel):
    ...
    _description = "Merge Purchase Automatic"
    ...
    def _update_values(self, src_purchase, dst_purchase):
        ...
        for po in src_purchase:
            self._add_message("a", [dst_purchase.name], po)

        po_names = src_purchase.mapped("name")
        self._add_message("desde", po_names, dst_purchase)
    ...
    def _add_message(self, way, po_name, po):
        ...
        """
        subject = _("Merge purchase order")
        body = _(
        ...
        po.message_post(
            body=body.format(way=way, po_names=" ,".join(po_name)),
            subject=subject,
