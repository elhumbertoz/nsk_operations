...
class EkReportAccountAgedPayable(models.AbstractModel):
    _name = "account.aged.payable.l10n_ec.balance.report.handler"
    ...
    def _report_custom_engine_l10n_ec_aged_receivable(
            self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None,warnings=None
    ):
        return self._l10n_ec_aged_partner_report_custom_engine_common(
    ...
    def _l10n_ec_aged_partner_report_custom_engine_common(
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                rslt.update({
                    'invoice_date': query_res['due_date'][0] if len(query_res['due_date']) == 1 else None,
...
class L10nEcAgedPayableCustomHandler(models.AbstractModel):
    _name = 'account.aged.payable.l10n_ec.report.handler'
    _inherit = 'account.aged.payable.l10n_ec.balance.report.handler'
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if options.get('account_type'):
            options['account_type'] = [
                account_type for account_type in options['account_type'] if account_type['id'] not in ('trade_receivable', 'non_trade_receivable')
            ]
...
class L10nEcAgedReceivableCustomHandler(models.AbstractModel):
    _name = 'account.aged.receivable.l10n_ec.report.handler'
    _inherit = 'account.aged.payable.l10n_ec.balance.report.handler'
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if options.get('account_type'):
            options['account_type'] = [
                account_type for account_type in options['account_type'] if account_type['id'] not in ('trade_payable', 'non_trade_payable')
            ]
...
class L10nEcAgedReceivableCustomSalespersonHandler(models.AbstractModel):
    _name = 'account.aged.receivable.salesperson.l10n_ec.report.handler'
    _inherit = 'account.aged.payable.l10n_ec.balance.report.handler'
    _description = 'Aged Receivable Sales grouped by salesperson'
