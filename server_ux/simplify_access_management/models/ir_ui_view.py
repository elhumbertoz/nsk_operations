...
class ir_ui_view(models.Model):
    ...
    def _postprocess_tag_div(self, node, name_manager, node_info):
        ...
        if name_manager.model._name == 'res.config.settings' and node.tag == 'app' and node.get('string'):
            for setting_tab in hide_button_obj.search([('access_management_id.company_ids', 'in', self.env.company.id),
                                                       ('model_id.model', '=', name_manager.model._name),
                                                       ('access_management_id.active', '=', True),
                                                       ('access_management_id.user_ids', 'in', request.env.uid)]).mapped(
    ...
    def _postprocess_tag_label(self, node, name_manager, node_info):
        ...
        if postprocessor:
            super(ir_ui_view, self)._postprocess_tag_label(node, name_manager, node_info)
            if node.get('for'):
                for hide_field in hide_field_obj.search([('access_management_id.company_ids','in',self.env.company.id),('model_id.model','=',name_manager.model._name),('access_management_id.active','=',True),('access_management_id.user_ids','in',request.env.uid)]):
                    for field_id in hide_field.field_id:
