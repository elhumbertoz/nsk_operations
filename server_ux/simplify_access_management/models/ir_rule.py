                                    ...
                                    for domain_tuple in res:
                                        if domain_tuple[0] == 'company_id':
                                            for company in access.access_management_id.company_ids.ids:
                                                if isinstance(all_company, list) and company in all_company:
                                                    all_company.remove(company)
                                            company_domain += ['|','&',('company_id','in',access.access_management_id.company_ids.ids)] + domain_list + [('company_id','in',all_company)]
                        if domain_list:                    
                            if company_domain:
                                return domain_list
                            return domain_list
