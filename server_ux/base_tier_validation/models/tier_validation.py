...
class TierValidation(models.AbstractModel):
    ...
    def _get_under_validation_exceptions(self):
        ...
        # TODO: Remover esto una vez que se revise y detecte con Yordany el problema.
        return ["message_follower_ids", "applied_coupon_ids","price_validate_ok", "credit_validate_ok", "access_token", "l10n_latam_check_reject_state", "in_requisition"]
    ...
    def write(self, vals):
        for rec in self:
            ...
            # TODO: Remover esto una vez que se revise y detecte con Yordany el problema.
            c1 = rec.review_ids
            c2_1 = getattr(rec, self._state_field)
            c2_2 = self._state_from
            c2 = c2_1 in c2_2
            c3_1 = vals.get(self._state_field)
            c3_2 = (self._state_to + [self._cancel_state])
            c3_3 = c3_1 not in c3_2
            c4 = rec._check_allow_write_under_validation(vals)
            c5 = rec._context.get("skip_validation_check")
            if (
                rec.review_ids
                and getattr(rec, self._state_field) in self._state_from
                and vals.get(self._state_field) not in (self._state_to + [self._cancel_state])
