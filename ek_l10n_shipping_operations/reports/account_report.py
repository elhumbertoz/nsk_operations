...
class EkReportAccountNskReceivableBalance(models.AbstractModel):
    _name = "account.aged.nsk.receivable.balance.l10n_ec.report.handler"
    _description = "Cuentas por Cobrar"
    _inherit = "account.report.custom.handler"
    


    def _report_custom_engine_l10n_ec_aged_payable(
            self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None,warnings=None
    ):
        return self._l10n_ec_aged_partner_report_custom_engine_common_nsk(
            options, 'liability_payable', current_groupby, next_groupby, offset=offset, limit=limit
        )

    def _report_custom_engine_l10n_ec_aged_receivable_nsk(
            self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None,warnings=None
    ):
        return self._l10n_ec_aged_partner_report_custom_engine_common_nsk(
    ...
    def _report_custom_engine_l10n_ec_aged_receivable_salesperson(
            self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None,warnings=None
    ):
        return self._l10n_ec_aged_partner_report_custom_engine_common_nsk(
            options, 'asset_receivable', current_groupby, next_groupby, offset=offset, limit=limit
    ...
    def _l10n_ec_aged_partner_report_custom_engine_common_nsk(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                rslt.update({
                    ...
                    'ship_name_id' : query_res['ship_name_id'][0] if query_res['ship_name_id'] else None,
                    'ship_name': query_res['ship_name'][0] if query_res['ship_name'] else None,
                    'journey_crew_id': query_res['journey_crew_id'][0] if query_res['journey_crew_id'] else None,
                    'journey_crew': query_res['journey_crew'][0] if query_res['journey_crew'] else None,
                    'ata' : query_res['ata'][0] if query_res['ata'] else None,
                    'atd' : query_res['atd'][0] if query_res['atd'] else None,
                    'company_id' : query_res['company_id'][0] if query_res['company_id'] else None,
                    'invoice_user_id': query_res['invoice_user_id'][0] if query_res['invoice_user_id'] else None,
      ...
      #  if current_groupby == 'ship_name_id':
       #     select_from_groupby = f"move.{current_groupby} AS grouping_key,"
        #    groupby_clause = f"move.{current_groupby}, {always_present_groupby}"
        if current_groupby:
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT account_move_line.invoice_user_id) AS invoice_user_id,
                ...
                ARRAY_AGG(DISTINCT account_move_line.company_id) AS company_id,
                 -- Información del barco
                ARRAY_AGG(DISTINCT move.ship_name_id) AS ship_name_id,
                ARRAY_AGG(DISTINCT ek_ship_name.name) AS ship_name,
                
                -- Información del viaje
                ARRAY_AGG(DISTINCT move.journey_crew_id) AS journey_crew_id,
                ARRAY_AGG(DISTINCT ek_journey_crew.name) AS journey_crew,
                ARRAY_AGG(DISTINCT ek_journey_crew.ata) AS ata,
                ARRAY_AGG(DISTINCT ek_journey_crew.atd) AS atd,
            ...
            LEFT JOIN ek_ship_registration  ek_ship_name ON ek_ship_name.id = move.ship_name_id
            LEFT JOIN ek_boats_information ek_journey_crew ON ek_journey_crew.id = move.journey_crew_id
        ...
        full_query = self.env.cr.mogrify(query, params)
        prinf_quey =full_query.decode('utf-8')
        print(prinf_quey)
            ...
            for line_to_expand in lines_to_expand: # In standard, this loop will execute only once
                if expand_function_name == '_report_expand_unfoldable_line_with_groupby':
                    ...
                    for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
                        ...
                        aml_data_by_partner = {}
                        for aml_id, aml_result in self._l10n_ec_aged_partner_report_custom_engine_common_nsk(
